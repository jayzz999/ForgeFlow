...
def _ensure_runnable(code: str, dag: "WorkflowDAG") -> str:
    ...
    LLMs sometimes truncate at max_tokens and leave the code without a main().
...
2. Use environment variables for ALL secrets (os.getenv with descriptive names like SLACK_BOT_TOKEN, GMAIL_APP_PASSWORD, etc.)
...
9. Make code FULLY SELF-CONTAINED in a single workflow.py — ALL client functions must be INLINE in the file
   NEVER create separate client modules — put ALL code in ONE file
...
def _fallback_code(dag: WorkflowDAG) -> str:
    ...
    for i, step in enumerate(dag.steps):
        ...
        if service.lower() == "http":
            url = step.inputs.get("url") or (step.api.base_url if step.api else "") or (step.api.endpoint if step.api else "")
            steps_code += f'''
    # Step {i+1}: {step.name}
    try:
        ...
        result = {{"ok": False, "status": "error", "url": {json.dumps(url)}, "error": str(e), "healthy": False}}
        print(json.dumps(result, indent=2))
        context["{step.id}"] = result
'''
            continue
...
'''
    ...
    return f'''# Auto-generated by ForgeFlow
import asyncio
import json
import httpx
import os
